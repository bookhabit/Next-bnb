...
const RegisterRoomBedrooms = () => {
    const maximumGuestCount = useSelector((state:any)=>state.registerRoom.maximumGuestCount)

    const bedroomCount = useSelector((state:any)=>state.registerRoom.bedroomCount)
    ...
    // 침실 개수 변경 시
    const onChangeBedroomCount = (event:React.ChangeEvent<HTMLSelectElement>)=>dispatch(registerRoomActions.setBedroomCount(getNumber(event.target.value)||0))
